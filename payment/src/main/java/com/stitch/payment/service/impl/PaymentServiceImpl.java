...
public class PaymentServiceImpl implements PaymentService{
    ...
    public PaymentServiceImpl(ProductOrderService productOrderService, TransactionService transactionService, ProductOrderService orderService, ProductOrderRepository productOrderRepository) {
    ...
    public void initializeOrder(InitializeTransactionRequest request){
        ...
        productOrderRequest.setStatus("PROCESSING");
        productOrderRequest.setUserId(email);
        productOrderRequest.setOrderId(request.getReference());
        productOrderRequest.setAmount(request.getAmount());
        productOrderRequest.setProductId(request.getProductId());
        productOrderRequest.setProductCategoryName(request.getProductCategoryName());
        productOrderRequest.setPaymentMode(PaymentMode.CARD);
        productOrderRequest.setNarration(request.getNarration());
        productOrderRequest.setVendorId(request.getVendorId());
        productOrderRequest.setQuantity(request.getQuantity());
...
//    @Transactional
    public InitializeTransactionResponse initTransaction(InitializeTransactionRequest request) throws Exception {
        System.out.println("entered scheduled method 1");
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String email = authentication.getName();
        System.out.println("entered scheduled method 2 " + email);
...
//    @Transactional
    public PaymentVerificationResponse paymentVerification(String reference) throws Exception {
        ...
        System.out.println("verify payment response reference : " +  reference);
        ...
        try{
            ...
            if (paymentVerificationResponse == null || !paymentVerificationResponse.getStatus()) {
                ...
                transaction = Transaction.builder()
                        ...
                        .status(paymentVerificationResponse.getStatus() ? TransactionStatus.C: TransactionStatus.F)
                ...
                ProductOrder productOrder = orderService.getOrderByOrderId(t.getTransactionId());

                log.info("productOrder entity: {}", productOrder);


                productOrder.setStatus(OrderStatus.COMPLETED);
